...
//            ->orderBy('tweet_images.idx')
            ...
            ->select(DB::raw("CONCAT(tweets.user_id, '/', tweets.id, '/', tweets.main_img) AS path, creators.nickname, tweets.id, tweets.include_video, tweets.file_cnt, users.account_id, CONCAT(tweets.user_id, '/', tweets.id, '/thumb_', SUBSTRING_INDEX(tweets.main_img, '.', 1), '.png') AS thumb_path, SUBSTRING_INDEX(main_img_mime_type, '/', 1) AS main_img_mime_type"))
            ->join('users', 'users.id', '=', 'tweets.user_id')
...
//        ]);
    ...
    public function timeline(Request $request, $account_id, $startTweet) {
        ...
        if ($follow === 1) {
            //입회한 사용자의 경우
            $query = "tweet_images.tweet_id, tweet_images.idx, CONCAT(tweets.user_id, '/', tweets.id, '/', tweet_images.name) AS path, tweet_images.mime_type, CONCAT(tweets.user_id, '/', tweets.id, '/thumb_', SUBSTRING_INDEX(tweet_images.name, '.', 1), '.png') AS thumb_path";
        }
        else {
            //입회하지 사용자의 않은 경우
