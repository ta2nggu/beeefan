...
function loadMoreData(page) {
    ...
    //21.04.17 김태영, admin/index.blade.php creator 정렬 기능을 위해 추가
    ...
    $.ajax({
        ...
        data:{'search':$search, 'sorting':$sorting, 'direction':$direction},
        beforeSend: function () {
            $(".ajax-load").show();
        }
    })
...
//21.04.17 김태영, 검색기능을 사용하게 되면 다시 loading 이미지를 보여줘야 하기 때문에
var ajaxLoadImgTag = $(".ajax-load").length > 0 ? $(".ajax-load").html() : "";

//21.04.17 김태영, post-data class 존재할 때만
...
//21.05.17 kondo, リダイレクトで「flash_message」表示に変更
