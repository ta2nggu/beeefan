            ...
            @role('user')
                <div class="helpBox">
                    ...
                    <span class="invalid-feedback" role="alert" style="margin-bottom: 30px">
                        <strong>{!! nl2br(session('flash_message')) !!}</strong>
                    </span>
                    @endif
                    ...
                    <p class="redBox">{{__('各ファンクラブの退会を個別に行った上で、当サイトの退会を行ってください。各ファンクラブ退会前に、当サイトの退会を行うことはできませんのでご注意ください。')}}</p>
                    ...
                    <form action="{{ route('removeAccount.post') }}" method="POST" class="formBox normalFormBox">
                        ...
                        <div class="formBox normalFormBox">
                            ...
                            <ul class="btnBox">
                                <li><a href="{{ route('pageHelp') }}" class="btn btnBl">{{ __('退会をキャンセル') }}</a></li>
                                <li><button disabled type="button" class="btn btnBor btnBorBl disabledBor" id="join_submit" data-toggle="modal" data-target="#removeDr">{{ __('退会する') }}</button></li>
                            </ul>
                        </div>
                        <!--退会確認ポップアップ-->
